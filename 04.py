...
pinput="""eyr:2029 byr:1931 hcl:z cid:128
...
passports = pinput.split('\n\n')

def parse_passport(passport_text):
  passport_text = passport_text.strip()
  fields_text=passport_text.replace('\n', ' ').split(' ')
  fields = dict([field.split(':') for field in fields_text])
  return fields

def is_valid(pf):
  return  'byr' in pf.keys() and \
     'iyr' in pf.keys() and \
     'eyr' in pf.keys() and \
     'hgt' in pf.keys() and \
     'hcl' in pf.keys() and \
...
def is_valid_height(height):
  if re.match(r'\d{2}in',height):
    ht = int(re.findall(r'\d+',height)[0])
    if ht >= 59 and ht <= 76:
      return True
  elif re.match(r'\d{3}cm',height):
    ht = int(re.findall(r'\d+',height)[0])
...
def is_valid2(pf):
  return  \
     'byr' in pf.keys() and int(pf['byr']) >= 1920 and int(pf['byr']) <= 2002 and \
     'iyr' in pf.keys() and int(pf['iyr']) >= 2010 and int(pf['iyr']) <= 2020 and \
     'eyr' in pf.keys() and int(pf['eyr']) >= 2020 and int(pf['eyr']) <= 2030 and \
     'hgt' in pf.keys() and is_valid_height(pf['hgt']) and \
     'hcl' in pf.keys() and re.match(r'^#[0-9a-f]{6}$',pf['hcl']) and \
     'ecl' in pf.keys() and re.match(r'amb|blu|brn|gry|grn|hzl|oth',pf['ecl']) and \
     'pid' in pf.keys() and re.match(r'^[0-9]{9}$',pf['pid'])

valid=0
for passport in passports:
  if is_valid2(parse_passport(passport)):
    valid+=1
print(valid)
