...
def parse_input(line):
  """
  Parse input line into rules and password
  """
  return [s.strip() for s in line.split(":")]

def password_matches(text):
  rule_text, password = parse_input(text)
  count, char = rule_text.split()
  at_least, at_most = count.split('-')
  at_least = int(at_least)
  at_most = int(at_most)
  counts = Counter(password)
  if char in counts and counts[char] >= at_least and counts[char] <= at_most:
    return password 
  
def password_matches2(text):
  rule_text, password = parse_input(text)
  count, char = rule_text.split()
...
if __name__ == "__main__":
 print(len( [password_matches2(l) for l in input.split('\n') if l and password_matches2(l)] ))
